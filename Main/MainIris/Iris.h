...
struct node {
	...
	vector<int> instances_of_each_type(vector<line> d) {
		...
		while (i < d.size()) {
			...
			else if (d[i].type == this->unique_types[2])type3++;
			++i;
		}
		return vector<int>{type1, type2, type3};
	}
	...
	void split_pre() {
		if (data.size() > 10) {
			...
			vector<int> type_count = instances_of_each_type(this->data);
			if (type_count[0] >= type_count[1] && type_count[1] >= type_count[2]) {
				attribute = -1;
			...
			else if (type_count[1] >= type_count[0] && type_count[1] >= type_count[2]) {
				attribute = -1;
	...
	void node_print() {
		...
		if (left == nullptr && right == nullptr) {
			...
			if (type_count[2] != 0) {
				cout << space << "Leaf: " << unique_types[2] << "(" << type_count[2] << ")" << endl;
			}
			return;
		}
